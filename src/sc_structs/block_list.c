...
void block_list_set(struct block_list_st *res, const struct block_list_st *a) {
    ...
    for (size_t i = 0; i < a->size; i++) res->blocks[i] = a->blocks[i];
}
...
void block_list_append(struct block_list_st *res, struct block_expr_st *data) {
    block_list_resize(res, res->size + 1);
    res->blocks[res->size - 1] = data;
}
void block_list_concat(struct block_list_st *res, const struct block_list_st *a) {
    if (res == NULL || a == NULL || a->size == 0) return;

    size_t _size = res->size;
    block_list_resize(res, res->size + a->size);
    for (size_t i = 0; i < a->size; i++) {
        res->blocks[_size + i] = a->blocks[i];
    }
}
